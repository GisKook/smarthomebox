...
unsigned char dataparse(Bussinessdata *data, unsigned char *buffer, int len)
{
	...
	data->status.Alarm2 = ( tempstatus & 0x02 );		
	data->status.Tamper = ( tempstatus & 0x04 );		
	data->status.Battery = ( tempstatus & 0x08 );		
	data->status.Supreprots= ( tempstatus & 0x10 );		
	data->status.Restorereps = ( tempstatus & 0x20 );		
	data->status.Trouble = ( tempstatus & 0x40 );		
	data->status.Acmains = ( tempstatus & 0x80 );
	bytebuffer_skipbyte(&tempdata);				
	//extendedStatus
